...
# pipeline joint framework node
class PipeJoint:
	def __init__(self):
		self.stageNum = 0 	# the stage of joint flop
		self.nodeInJoint = []	# number of flops for each stage joint
	def __del__(self):
		pass	


# SOLVER MODULE: GENERATION V1.1 : Level Based Timing Generation Solver Module
# Generate the matrix equivalent for the cvx-opt module v1.1 : first Iteration : Level Based Timing Generation
...
# Buffer Modeling feature added : 
# 	Rest of the gates have been modeled with their associated wires
def solverOptLevelBased(dictNode,stageModule,minNodeIn,minNodeOut,numVar,estFlop,numStage):
	#print "\n The number of stages is: ",numStage
	...
	power = gpower()
	flop = mflop()
	# Considering that numVar is the number of nodes, In simplistic model of optimization (AT & x): nVar = numVar + numStage
	nVar = numVar + numStage	# no.gates & no.arrival times(level based)
	addBuff = numVar - 1
		...
		for j in range(len(stageModule[i].gateList)):	
			...
			for m in range(len(dictNode[SelfGate].fanInList)):
				...
				#<1>	# monomial : __inequality map__
					...
					ma = matrix((0.0),(nVar,1)) 
					ma[addBuff + dictNode[SelfGate].gateStage] = -1		# set the variable : exponent value
					mf = matrix([[mf],[ma]])				# F matrix: concatenate min-array ma to mf			
				...
				#<4>	# monomial: inequality map
					ma = matrix((0.0),(nVar,1)) 
					ma[addBuff + dictNode[dictNode[SelfGate].fanInList[m]].gateStage] = 1		# set the variable : exponent value
					ma[addBuff + dictNode[SelfGate].gateStage] = -1					# set the variable : exponent value
					mf = matrix([[mf],[ma]])							# F matrix: concatenate min-array ma to mf										
	...
	result = exp(sol['x'])
	endSolve = time.time() 
	insertTimeStamp(filename,lineno(),'solOpt_level_gp','solverOpt_level_gp:['+str(endSolve-startSolve)+']')
	
	if sol['status'] == 'optimal':
		sline = ' module1.1: optimization succes! '
		skey = 'opt1.1 succes'
		insertStatus(filename,lineno(),skey,sline)
		# store the dimension
		dime = mf.size 
		sline = " opt1.1 Mode: variables "+ str(dime[0])+ " monomials "+ str(dime[1])+ " inequalities "+ str(len(mk)) 
		skey = 'opt1successDim'
		insertStatus(filename,lineno(),skey,sline)
	else:
...
# SOLVER MODULE: GENERATION V2.1
# Generate the matrix equivalent for the cvx-opt module v2.1 : second Iteration # gateStage
# 	The matrix generation is followed according the specified matrix tablulation as mentioned as sample on cvx-opt website		
...
def solverOptSeqLevelBased(dictNode,stageModule,minNodeIn,minNodeOut,numVar,estFlop,SeqBlockJoint ,numStage):
	# Note: each monomial is a column of the matrix: F and constant associated in g and posynomial factor in K
	# the generation for each monomial will be based on the number of variables
	# the number of variables is equal to the number of sizing variables and the arrival time: 
	# minimalistic design framework	
	...
	result = exp(sol['x'])
	if sol['status'] == 'optimal':
		sline = ' module2.1: optimization succes! '
		skey = 'opt2.1succes'
		insertStatus(filename,lineno(),skey,sline)
		# store the dimension
		dime = mf.size 
		sline = " opt2.1 Mode: variables "+ str(dime[0])+ " monomials "+ str(dime[1])+ " inequalities "+ str(len(mk)) 
		skey = 'opt2.1successDim'
		insertStatus(filename,lineno(),skey,sline)
	else:
...
def populatedictNodeTable(dictNode,stageModule,result,numVar,status):
	for i in range(0,len(stageModule)):			
		for j in range(len(stageModule[i].gateList)):
			SelfGate = stageModule[i].gateList[j]
			dictNode[SelfGate].solveGateSize = result[dictNode[SelfGate].gateSerNum]		# store gate-sizing in dict
			dictNode[SelfGate].solveArrivalTime = result[numVar-1 + dictNode[SelfGate].gateStage] 	# store arrival time in dict
			if status == 'comb':
				dictNode[SelfGate].pipeJoint = 0
...
def displayresultSolver(dictNode,stageModule,minNodeOut):
	...
	for i in range(0,len(stageModule)):	
		...
		for j in range(len(stageModule[i].gateList)):	
			...
			# gate delay
			dictNode[SelfGate].gateTime = power.retAlphaPowerDelayTime(dictNode[SelfGate].unitRes,dictNode[SelfGate].solveGateSize,cap)
	...
	if isSequentialMode:
		...
		print "Combinational Block Optimization : Level Based Timing"
...
def generateSequentialBlockFramework(dictNode,stageModule,SeqBlockJoint ,maxStage):
	...
	if pipeDef < const.numPipeStage - 1: # less pipelined stages than requires
		sline = "Ceems2: Fail:less stage pipelined"
		skey = 'ceemsIIgenCut'
		insertStatus(filename,lineno(),skey,sline)	
	elif pipeDef < const.numPipeStage - 1: # more pipelined stages than requires
		sline = "Ceems2: Fail:excess stages pipelined"
		skey = 'ceemsIIgenCut'
		insertStatus(filename,lineno(),skey,sline)		
	else:
		sline = "Ceems2: Pipeline: Required Stages Achieved"
		skey = 'ceemsIIgenCut'		
		insertStatus(filename,lineno(),skey,sline)
...
def generateAndSolveOptSeq(dictNode,stageModule,minNodeIn,minNodeOut,numVar,estFlop,SeqBlockJoint ,numStage):
	# generate for joint combinational block optimization
	result = solverOptSeqLevelBased(dictNode,stageModule,minNodeIn,minNodeOut,numVar,estFlop,SeqBlockJoint ,numStage)
	populatedictNodeTable(dictNode,stageModule,result,numVar,'seq')						# plug in the solver based results into the dict table	
...
# test module for analysis the stage timing map
def  extractTimingStageMap(result,numStage,numVar):
	line = ''
	for i in range(numVar,len(result)):
		line += 'AT['+str(i-numVar+1)+']: '+str(result[i]) + ' '
	insertStatus(filename,lineno(),'sol_stage_gp_time','stage_time:' + line)	
			
			
# experimental phase: stagebased pipeline design framework : overwrites prior design framework
def levelBasedOptimizationFramework(dictNode,stageModule,minNodeIn,minNodeOut,numVar,estFlop,SeqBlockJoint ,numStage):
	result = solverOptLevelBased(dictNode,stageModule,minNodeIn,minNodeOut,numVar,estFlop,numStage)	
	#extractTimingStageMap(result,numStage,numVar)
	populatedictNodeTable(dictNode,stageModule,result,numVar,'comb')
	displayresultSolver(dictNode,stageModule,minNodeOut)
	updateTimingMapforGateSize(dictNode,stageModule,minNodeIn,minNodeOut)
	# sequential process :
	#flopCount, maxStage = determinePipelineJoints(dictNode,stageModule,minNodeIn,minNodeOut)	
	#generateSequentialBlockFramework(dictNode,stageModule,SeqBlockJoint ,maxStage)
	#determinePadPipeDesignFeasible(dictNode,stageModule,minNodeIn,minNodeOut,maxStage,flopCount)	
	#generateAndSolveOptSeq(dictNode,stageModule,minNodeIn,minNodeOut,numVar,estFlop,SeqBlockJoint ,numStage)
	#adjustMinPathDelayIterativeSolver(dictNode,stageModule,minNodeIn,minNodeOut)
